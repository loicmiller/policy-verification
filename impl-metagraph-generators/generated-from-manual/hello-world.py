...
if __name__ == "__main__":
	...
	rule_head = 'hello'
	if "m:=input.message" not in propositions:
		propositions.append("m:=input.message")
	if "m:=input.message" not in current_propositions:
		current_propositions.append("m:=input.message")
	if "m==\"world\"" not in propositions:
		propositions.append("m==\"world\"")
	if "m==\"world\"" not in current_propositions:
		current_propositions.append("m==\"world\"")
	src = "rule_1"
	variables.append(src)
	dst = rule_head
	edges.append(Edge({src}, {dst}, attributes=current_propositions))
	...
	print(repr(cm))
	g = Digraph("hello-world", filename="../metagraphs/hello-world.gv")
